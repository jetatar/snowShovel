...
const UInt_t NSnCRCUtils::crc_tabxfer_normal[256] = {
...
UInt_t NSnCRCUtils::GetCRC32for(FILE* inf) {
   // calculate the 32bit CRC of data in the file from the current
   // position onward. caller should make sure the current position
   // is the start of the file, if that is desired.
   // this function does not move the current file position back
   // to where it was when the function was called.
   //
   // returns 0 on failure
   
   UInt_t crc(0);
   if (inf!=0) {
      int c; // needs to be int for the EOF possibility
      while ( (c=getc(inf))!=EOF ) {
         crc = update_crc32_xfer(crc, static_cast<UChar_t>(c));
      }
   } else {
      Printf("<NSnCRCUtils::GetCRC32for>: Cannot get CRC for null file ptr.");
   }
   return crc;
}

UInt_t NSnCRCUtils::GetCRC32for(const Char_t* infn) {
   FILE* inf = fopen(infn, "rb");
   UInt_t crc(0);
   if (inf!=0) {
      crc = GetCRC32for(inf);
   } else {
      throw std::runtime_error(Form("<NSnCRCUtils::GetCRC32for>: "
