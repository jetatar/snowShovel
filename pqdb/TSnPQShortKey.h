#ifndef SNS_TSnPQShortKey
#define SNS_TSnPQShortKey
...
//
//
// Wrapper for a TSnPQSimpleKey<Short_t>. This is useful because rootcint
...
class TSnPQShortKey : public TSnPQDbKey {
 private:
   TSnPQSimpleKey<Short_t>*  fSkey;
 ...
 public:
   TSnPQShortKey() : fSkey(0) {}
   TSnPQShortKey(const Short_t k, const Char_t* table,
                 const Char_t* kcol, const Char_t* vcol,
                 const NSnPQSimpleKey::EValType vtype);
   virtual ~TSnPQShortKey();
   ...
   ClassDef(TSnPQShortKey, 1); // simple key to select one column using a short
};

#endif //SNS_TSnPQShortKey
