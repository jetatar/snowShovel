...
TObjArray* hSigCutoffs(0);              // [channel][event]
...
void makeWaves(const Char_t*  infn,
   ...
   //"/home/cjreed/work/temp/hists.temp.ev_All.dat.gz.root",
               const Char_t*  FPNfn=NULL,  // PEDS file
               const Int_t    stopAlgoAmpOn=2, // 0=getStopLocs,
                                               // 1=AveSmpSpreadGroups
                                               // 2=SignalCutoff
               ...
               const Int_t    rmsStpSmps=4,  // getStopsFromSampleSpread stpNumSmp
               const Float_t  rmsRelWin=2.0, // getStopsFromSampleSpread relWin
               const Bool_t   assumeAmpsOn=kTRUE,
               const Char_t*  configdb=0,
               const Char_t*  ntname="runtree"
               ) {
   ...
   TDirectory* hssdir  = outf->mkdir("SampleSpreads");
   ...
   TDirectory* powdir  = outf->mkdir("PowerReadings");
   TDirectory* rundir  = outf->mkdir("DAQRunNums");
   TDirectory* seqdir  = outf->mkdir("DAQSeqNums");
   ...
   hSmpSpread          = new TObjArray(nents);
   ...
   TDirectory* sctdir=0;
   if (stopAlgoAmpOn==2) {
      sctdir           = outf->mkdir("SigCutoffs");
      hSigCutoffs      = new TObjArray(kNchans); // [chan][event]
   }
   ...
   Double_t mean, rms;
   UShort_t smp;
   UInt_t runnum; UShort_t seqnum; TString hv;
   for (Long64_t e=0; e<nents; ++e) {
      ...
      runnum = (extra[31]<<16)+extra[30];
      seqnum = extra[35];
      
      TParameter<Int_t> runp(Form("DAQRunNum_ev%06lld",e),
                             static_cast<Int_t>(runnum));
      rundir->WriteObject(&runp, runp.GetName());
      
      TParameter<Int_t> seqp(Form("DAQSeqNum_ev%06lld",e),
                             static_cast<Int_t>(seqnum));
      seqdir->WriteObject(&seqp, seqp.GetName());
      
      hv = Form("V1_r%05d_s%05d",runnum,seqnum);
      TParameter<Int_t>* pv1 = dynamic_cast< TParameter<Int_t>* >(
         powdir->Get(hv.Data()));
      if (pv1==0) {
         pv1 = new TParameter<Int_t>(hv.Data(), 
            static_cast<Int_t>(extra[40]));
         powdir->WriteObject(pv1, pv1->GetName());
         delete pv1; pv1=0;
      }
      hv = Form("V2_r%05d_s%05d",runnum,seqnum);
      TParameter<Int_t>* pv2 = dynamic_cast< TParameter<Int_t>* >(
         powdir->Get(hv.Data()));
      if (pv2==0) {
         pv2 = new TParameter<Int_t>(hv.Data(), 
            static_cast<Int_t>(extra[41]));
         powdir->WriteObject(pv2, pv2->GetName());
         delete pv2; pv2=0;
      }
   ...
   */
   ...
   for (Int_t ch=0; ch<kNchans; ++ch) {
      ...
      if (gr==0) {
         ...
         gr->SetTitle(Form("Average Sample, RMS (chan %d);sample;ADCs",ch));
   ...
   for (Int_t ch=0; ch<kNchans; ++ch) {
      ...
      hfa = new TH1F(hn.Data(),
         ...
         kHalfsamps+1, -0.5*kfincr, (kHalfsamps+0.5)*kfincr );
      hAveFreqs->AddAt(hfa, ch);
   }
   
   Int_t ev=0;
   Int_t shift[kNchans]={0};
   Int_t lens[kNchans]={0};
   nShifts = new TTree("nShifts","nShifts");
   nShifts->Branch("Ent",&ev,"Ent/I");
   nShifts->Branch("EvtNum",&evnum,"EvtNum/i");
   for (Int_t ch=0; ch<kNchans; ++ch) {
      hn = Form("shift%02d",ch);
      nShifts->Branch(hn.Data(), &(shift[ch]),
                      Form("%s/I",hn.Data()));
      hn = Form("len%02d",ch);
      nShifts->Branch(hn.Data(), &(lens[ch]),
                      Form("%s/I",hn.Data()));
   }
   nShifts->SetDirectory(outf);

   UChar_t ampon = (assumeAmpsOn) ? ~0 : 0;
   
   for (ev=0; ev<nents; ++ev) {
      nt->GetEntry(ev+firstEvt);
      if ( (ev%100)==0 ) {
      ...
      // find sample spreads
      TH1** hpswvs = new TH1*[kNchans];
      for (Int_t ch=0; ch<kNchans; ++ch) {
         TObjArray* apswvs = 
            dynamic_cast<TObjArray*>(hPedSubWvfm->At(ch));
         if (apswvs==0) {
            Fatal("find stops","hPedSubWvfm empty at ch=%d",ch);
         }
         // get individual waveform
         hpswvs[ch] = dynamic_cast<TH1F*>(apswvs->At(ev));
      }
      TString hsn(Form("hSmpSpread_ev%06d",ev));
      TString hst(Form("RMS of ADC-FPN values accross channels"
                       " (ev %d);sample;RMS",ev));
      TH1* hss = newSampleSpreadHist(hsn.Data(), hst.Data(),
                                     hpswvs, kNchans);
      hSmpSpread->AddAt(hss, ev);
      delete[] hpswvs;
      ...
      for (Int_t ch=0; ch<kNchans; ++ch) {
	   ...
	   if (fpnf!=0 && fpnf->IsZombie()==kFALSE) {
		...
		// amp on
                 if (stopAlgoAmpOn==0) {
                    getStopLocs( h1f->GetArray() + 1, // don't include underflow
                                 ...
                                 stpRelWin,
                                 kNsamps );
                } else if (stopAlgoAmpOn==1) {
                    if (ch==0) {
                       TH1* hdum = newAveSmpSpreadGroupsHist("hdum","dummy",
                                                             *hss,6);
                       stops[ch].push_back( LocLen_t(hdum->GetBinCenter(
                                                        hdum->GetMinimumBin())
                                                     -1, // stop in [0, N)
                                                     6) );
                       delete hdum;
                       ...
                       stops[ch].assign(stops[0].begin(), stops[0].end());
                    }
                } else if (stopAlgoAmpOn==2) {
                    static const Int_t nsmp = 6;
                    hsn = Form("hSigCutoff_ch%02d",ch);
                    hst = Form("signal cutoff ratio (chan %d);"
                               "sample;ave %d after inc. / ave %d before exc.",
                               ch, nsmp, nsmp);
                    TH1* hsc = newSignalCutoffHist(hsn.Data(), hst.Data(), 
                                                   *h1f, nsmp);
                    stops[ch].push_back( LocLen_t(hsc->GetBinCenter(
                                                     hsc->GetMinimumBin())
                                                  -1, // stop in [0, N)
                                                  nsmp) );
                    if (hSigCutoffs!=0) {
                       TObjArray* scch = dynamic_cast<TObjArray*>(
                          hSigCutoffs->At(ch));
                       if (scch==0) {
                          scch = new TObjArray(nebins-firstEvt);
                          hSigCutoffs->AddAt(scch, ch);
                       }
                       scch->AddAt(hsc, ev);
                    } else {
                       delete hsc;
                    ...
                    Fatal("find stops","Unhandled value of stopAlgoAmpOn = [%d]",
                          stopAlgoAmpOn);
                }
		...
		bestStops[ch] = (stops[ch].empty()) ? LocLen_t(0,0) :
            ...
            shift[ch] = bestStops.at(ch).first;
            lens[ch]  = bestStops.at(ch).second;
   ...
   writeAndClean(hSmpSpread, hssdir);
   ...
   if (hSigCutoffs!=0) {
      writeAndClean(hSigCutoffs, sctdir);
   }
   nShifts->Write(nShifts->GetName(),TObject::kOverwrite);
   delete outf;
   delete[] ampInf;
