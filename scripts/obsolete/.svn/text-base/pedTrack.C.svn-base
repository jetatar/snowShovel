...
void pedTrack(const Char_t* infn,
              const Char_t* outfn,
              const Int_t   trg=2, // BIT(1)
              const Bool_t  verifyChksum=kTRUE,
              const Float_t peakFrc=0.05,
              const Float_t ignoreExtremeAdcs=kTRUE,
...
/*
      if (ok) {
         for (Int_t ch=0; ch<kNchans; ++ch) {
            ok = (40 > TMath::RMS(kNsamps, &(samples[ch][0])));
         }
      }
*/      
      if (ok) {
   ...
   for (Int_t ch=0; ch<kNchans; ch++) {
      ...
      hWvVsT[ch] = new TH2F(hn.Data(),
         Form("waveform adc values vs time (chan %d);Date, Time (UTC);ADC",
              ch),
         ntimebins, mintime, maxtime,
         nadcbins, adcmin, adcmax);
      ...
      for (Int_t sm=0; sm<kNsamps; sm++) {
         ...
         findMeanStepFromPeak(*h, peakFrc, mean, rms, n, ignoreExtremeAdcs);
   ...
   for (Long64_t e=0; e<nents; e++) {
      ...
      if (gevts[e]) {
         ...
         for (Int_t ch=0; ch<kNchans; ch++) {
            ...
            for (Int_t sm=0; sm<kNsamps; sm++, ped++) {
